...
#define CENTER_FREQ  (82) //»ù×¼ÆµÂÊ£¨82-64£©*10+240+(3) = 423MHZ
...
//	SPI_RW_Reg(W_REG | 0x33, 0x42);  //使用Header byte 0,1,2,3 为头码  /  同步字使用 3,2 
	...
	//433信道偏移,1M
		SPI_RW_Reg(W_REG | 0x75, CENTER_FREQ); 
	#ifdef SI4432_CH
		SPI_RW_Reg(W_REG | 0x79, (SI4432_CH-73)*10);
	#else
		SPI_RW_Reg(W_REG | 0x79, (Channel_Get()-73)*10);  
	#endif
	...
	#else
		SPI_RW_Reg(W_REG | 0x75, Channel_Get());  
	#endif
...
static u8 Check_433M_Busy(u8 level)
{
	const static u16 randWait_Tab[] = {5, 5, 5, 5};//{5, 30, 200, 2000};
