...
//static u8 sendBuf[FIFOBUFSZE];    /**> 以前定义 **/
...
//	fifo_Init(&send433fifo, sendBuf, FIFOBUFSZE);          /**> 修改发送逻辑 **/
	rcv_433.flag = 0;
...
//	Thread_Login(FOREVER, 0, 5000, &ClearRepeatSerialNum);//周旺增加重复流水号，增加超时判断，5秒一次就好。2015.7.30
}
...
void Process_FIFOData(FIFO_t *fifo, Process_t *prc)
{
	switch (prc->flag)
	{
		...
		default:	
					...
					Thread_Login(ONCEDELAY, 0, 60000*5, Upload);		//空闲时主动上报，防止无线死机
#endif 
...
返 回 值：	无
*******************************************************************/
void Process_SendFIFOData(Process_t *prc)
{
	switch (prc->flag)
	{
		case 0:	if (0 == (prc->len = cmd_get_data(prc->data))) //从FIFO中获取一条指令失败
				{
					break;
				}
				prc->mean = (prc->con == MANY_P ? gSysData.sendCnt : 1);//获取数据处理次数
...
void Msg_Process(u8 *sdata, u8 len, Communication_t *pMsg, u8 *data)
{
	...
	if ((result = Object_Check(sdata, len, &pMsg->aim, &pMsg->para, data)) == COMPLETE) //目标核对成功
	{
		...
		if (result != NO_ACK)
		{
#ifdef  STM32L151		//专用于低功耗产品，休眠时间重置
			StandbyCountReset();
#endif 
			Instruct();		//信号指示
