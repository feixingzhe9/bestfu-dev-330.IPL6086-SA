...
BF_INT16U ChkCrcValue(BF_INT08U *buf, BF_INT16U len)
{
    BF_INT16U i;
    BF_INT08U	index;
    BF_INT08U	CRCH = 0;						//以前是0xFF,由于上位机改成了0，此处改成0
    BF_INT08U	CRCL = 0;						//以前是0xFF,由于上位机改成了0，此处改成0

    for( i = 0 ; i < len ; i++ )  				//传输消息缓冲区
    {
        index = CRCH ^ buf[i];
        CRCH  = (BF_INT08U)CRCL ^ (BF_INT08U)CRCTable[index];
...
BF_INT16U ChkCrcValueEx(BF_INT08U *buf, BF_INT16U len, BF_INT16U FirstData)
{
    ...
    CRCH = (BF_INT08U)(FirstData>>8);
    CRCL = (BF_INT08U)(FirstData);
