...
MsgResult_t Set_ManyUnit_Attr(UnitPara_t *pData)
{
    if (pData->len > 0)  	//核对参数长度
    {
        MsgResult_t result;
        BF_INT08U buf[128];
        BF_INT08U paraLen = pData->len;	//参数长度
        BF_INT08U *pPara = (BF_INT08U*)&pData->data;	//参数区指针
        ...
        while (pPara < &pData->data[paraLen])   //参数区没有使用完
        {
            ...
            {
                *(BF_INT16U*)&pData->data[0] = *(BF_INT16U*)&buf[1];  //赋值单元号与属性号
...
MsgResult_t Get_ManyUnit_Attr(UnitPara_t *pData, BF_INT08U *rlen, BF_INT08U *rpara)
{
    *rlen = 0;	//返回数据长度清零
    ...
    {
        BF_INT08U para[255]; //参数区
        BF_INT08U paraLen = pData->len;
        BF_INT08U *pPara = &para[paraLen];	//参数区指针
        BF_INT08U *aimPara = para;			//下传消息参数
        ...
        while (aimPara < &para[paraLen])
        {
            ...
            if (COMPLETE == Msg_Execute((CMDPara_t*)pPara, pData->msgsrc))  	//如果消息完成
            {
    ...
    {
        BF_INT08U data[5],ExeAttrNum = 0u; 		//参数区
        const AttrExe_st *pAttrExe = UnitTab[pData->unit].attrExe;
        do
        {
            if (LEVEL_1 == pAttrExe->level)  	//如果属性等级较高
            {
                data[0] = pData->msgsrc;	//消息源
				data[1] = pData->unit;		//单元
                data[2] = pAttrExe->cmd;	//属性
                data[3] = 0;				//参数长度
                if ((pAttrExe->pGet_Attr != NULL)
                        && (COMPLETE == pAttrExe->pGet_Attr((UnitPara_t*)data, &data[4], &rpara[(*rlen)+3]))	//如果获取属性内容完成
                   )
                {
                    rpara[(*rlen)++] = pData->unit;		//单元
                    rpara[(*rlen)++] = pAttrExe->cmd;	//属性
                    rpara[(*rlen)++] = data[4];			//参数长度
                    *rlen += data[4];
